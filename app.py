...
# ???? ??????? ??????? (???????? ???, ??? ???? ?? ????? ????? ????? ?????)
host_address = st.sidebar.text_input("????? ???? - ?????", value=default_video)
input_host_address = st.sidebar.text_input("????? ???? - ???", value=default_input)
...
if st.session_state.streaming and host_address:
    ...
    def send_click_command(cx, cy):
        try:
            ...
            st.toast(f"????? ?????: X={cx}, Y={cy}")
        except Exception as ex:
            st.error(f"?????: {ex}")

    with b_col1:
        if st.button("?? ?????"): send_click_command(200, 500)
    with b_col2:
        if st.button("?? ????"): send_click_command(500, 500)
    with b_col3:
        if st.button("?? ?????"): send_click_command(800, 500)
